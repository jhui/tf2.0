...
# Load a dataset for building facades
...
def load(image_file):
  ...
  # image (256, 512, 3) w = 512
  w = tf.shape(image)[1]

  # 256
  w = w // 2

  # real_image, input_image (256, 256, 3)
  # real_image has the full color &
  # input image is the cartoon like image.
  real_image = image[:, :w, :]
  input_image = image[:, w:, :]
...
def Discriminator():
  ...
  inp = tf.keras.layers.Input(shape=[None, None, 3],
                              name='input_image')
  tar = tf.keras.layers.Input(shape=[None, None, 3],
                              name='target_image')
...
def fit(train_ds, epochs, test_ds):
  for epoch in range(epochs):
    ...
    # Test on the same image so that
    # the progress of the model can be
    # easily seen.
