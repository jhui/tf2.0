...
# Load the dataset for the horse and the zebra
...
# discriminator_y(sample_zebra) (1, 30, 30, 1)
plt.imshow(discriminator_y(sample_zebra)[0, ..., -1],
           cmap='RdBu_r')

plt.subplot(122)
plt.title('Is a real horse?')
plt.imshow(discriminator_x(sample_horse)[0, ..., -1],
           cmap='RdBu_r')
...
def generator_loss(generated):
  # generated (1, 30, 30, 1) , return a scalar
  return loss_obj(tf.ones_like(generated), generated)

# If we pass through generator G & F (or vice versa)
# We should reconstruct the original image.
def calc_cycle_loss(real_image, cycled_image):
    # real_image, cycled_image (1, 256, 256, 3)
...
# If we pass Y to generator G,
# we should get Y even not that obvious.
# Same for passing X to generator F.
def identity_loss(real_image, same_image):
  # real_image, same_image (1, 256, 256, 3)
...
def train_step(real_x, real_y):
    # persistent is set to True because
    # the tape is used more than
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        # Want the generator to fool the discriminator.
        gen_g_loss = generator_loss(disc_fake_y)
        gen_f_loss = generator_loss(disc_fake_x)

        # Reconstruction loss passing through G and F
